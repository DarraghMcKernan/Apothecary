...
public class FollowMouse : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (workbenchActive == true && Input.GetMouseButton(0) && worldPos.x <= transform.position.x + sizeObject && worldPos.x >= transform.position.x - sizeObject && worldPos.z <= transform.position.z + sizeObject && worldPos.z >= transform.position.z - sizeObject)
        {
